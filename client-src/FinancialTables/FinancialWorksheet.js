...
function parseTableData(financialWorksheetData){

    console.log(financialWorksheetData);
    ...
    let data = new Body(0);
    for (let i = 0; i < financialWorksheetData.length; i++){
        let rec = new Cols(financialWorksheetData[i], {head: worksheetHead});
...
function importProc({FINANCIAL_WORKSHEET, BALANCE, savedFinancialWorksheet}){
    ...
    let financialWorksheetData, date;
    if(savedFinancialWorksheet.data.length > 0 || Object.keys(savedFinancialWorksheet.data).length > 0){
        let {date: savedDate, content: savedContent} = savedFinancialWorksheet.data;
        console.log(savedDate, 'saved');
        financialWorksheetData = parseTableData(savedContent);
        date = dateHead.createBody([savedDate])
    } else {
        console.log(FINANCIAL_WORKSHEET)
        financialWorksheetData = parseTableData(FINANCIAL_WORKSHEET.data);
        date = dateHead.createBody([{year: 2014, startPeriod: 0, endPeriod: 15}])
    }

    if (financialWorksheetData.error){
        // error handeling
    ...
    return [
        ...
        new Table(worksheetHead, financialWorksheetData, {expandable: true, autoExpanded: true, editable: true})
    ]    
...
export default function(){ 
    return new Sheet({
        referred: {
            FINANCIAL_WORKSHEET: {desc: '资产负债表表底稿', location:'remote'},
            BALANCE: {desc: '科目余额', location:'remote'},
            savedFinancialWorksheet: {desc: '已保存的资产负债表底稿模版', location:'remote', type:'CONF'}
        },
        importProc,
        exportProc,
        desc: '资产负债表量表底稿',
