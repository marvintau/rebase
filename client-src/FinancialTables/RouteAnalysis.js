...
function makeBackwardReference(listOfList, childKey="heir"){
    let backward = new List(0);
    ...
    for (let outerIndex = 0; outerIndex < listOfList.length; outerIndex++){
        ...
        for (let innerIndex = 0; innerIndex < innerList.length; innerIndex++){
            let line = innerList[innerIndex];
            line[childKey] = innerList;
            backward.push(line);
        }
    }
    return backward;
...
function getVoucherDict(voucherData){
...
function importProc({BALANCE, JOURNAL}){
    ...
    let voucherDict = getVoucherDict(voucherData);
    ...
    let balanceData = List.from(BALANCE.data)
        ...
        .iter((key, balance) => {
            let vouchers = voucherDict.get(key);
